...
import { EVENT, TICKET_TIERS } from './config';
import { formatCurrency } from './utils';
import { buildTicketQrPayload } from './qr';

const resend = new Resend(process.env.RESEND_API_KEY || "re_dummy");

// Resolve tier label from ticketTierId
function getTierLabel(ticketTierId: string): string {
  const tier = TICKET_TIERS.find((t) => t.id === ticketTierId);
  return tier?.label ?? ticketTierId;
}

// Build a hosted QR code image URL (works in all email clients incl. Gmail)
function buildQrImageUrl(qrPayload: string): string {
  const encoded = encodeURIComponent(qrPayload);
  return `https://api.qrserver.com/v1/create-qr-code/?size=220x220&margin=10&ecc=H&data=${encoded}`;
}
...
export async function sendTicketEmail(order: any) {
  ...
  try {
    ...
    const tierLabel = getTierLabel(order.ticketTierId);

    // Build QR payload and generate a hosted image URL (Gmail-safe)
    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
                ...
                <tr>
                  <td width="50%" style="padding:10px 10px 10px 0;border-bottom:1px solid rgba(212,175,55,0.08);">
                    ...
                    <p style="margin:0;font-size:14px;color:#ede6da;">${tierLabel} × ${order.quantity}</p>
                  </td>
                  <td width="50%" style="padding:10px 0 10px 10px;border-bottom:1px solid rgba(212,175,55,0.08);">
          ...
          <tr>
            <td style="background-color:#0f0c14;border-left:1px solid rgba(212,175,55,0.3);border-right:1px solid rgba(212,175,55,0.3);padding:0 24px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="20"><div style="width:20px;height:20px;background:#06050a;border-radius:50%;border-right:1px solid rgba(212,175,55,0.3);"></div></td>
                  <td style="border-top:2px dashed rgba(212,175,55,0.25);height:1px;"></td>
                  <td width="20"><div style="width:20px;height:20px;background:#06050a;border-radius:50%;border-left:1px solid rgba(212,175,55,0.3);"></div></td>
                </tr>
              </table>
            </td>
          </tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    ...
                    <!-- QR Code via hosted API — renders in all email clients including Gmail -->
                    <div style="display:inline-block;background:#ffffff;padding:12px;border-radius:4px;border:2px solid #d4af37;">
                      <img src="${qrImageUrl}" alt="Your Ticket QR Code" width="220" height="220" style="display:block;width:220px;height:220px;border:0;" />
                    </div>
          ...
          <tr>
            <td align="center" style="padding:28px 16px;">
              <p style="margin:0 0 16px;font-size:13px;color:#9a948c;">View your full digital ticket anytime:</p>
              <a href="${ticketUrl}" style="display:inline-block;background:#d4af37;color:#0b0b0d;padding:14px 32px;text-decoration:none;font-weight:bold;font-size:13px;text-transform:uppercase;letter-spacing:2px;border-radius:2px;">
                View My Ticket →
              </a>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:0 16px 24px;">
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#0f0c14;border:1px solid rgba(212,175,55,0.1);border-radius:4px;padding:20px 24px;">
                <tr>
                  <td style="padding:0 0 12px;">
                    <p style="margin:0 0 4px;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#5e5a55;">📅 Event Details</p>
                    <p style="margin:0;font-size:13px;color:#ede6da;">${EVENT.date} &nbsp;·&nbsp; ${EVENT.venue}</p>
                  </td>
                </tr>
                <tr>
                  <td>
                    <p style="margin:0 0 4px;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#5e5a55;">⚠️ Important</p>
                    <p style="margin:0;font-size:12px;color:#9a948c;line-height:1.6;">
                      Bring a valid government-issued photo ID. Tickets are non-transferable — name on ticket must match your ID.
                    </p>
                  </td>
                </tr>
              </table>
